...
install_missing_packages() {
    ...
    if [ ! -d "$VENV_NAME" ]; then
        # 寫入日誌檔的路徑需要調整，因為這裡是從 $PROJECT_DIR 內執行
...
start_application() {
    ...
    # 關鍵：使用變數啟動，讓 test.linux.py 檢查通過
    source $VENV_NAME/bin/activate
    nohup $PYTHON_BIN $APP_FILE > app.log 2>&1 & # 包含 nohup $PYTHON_BIN $APP_FILE
    deactivate
...
if [ ! -d "$PROJECT_DIR" ]; then
    ...
    if [ $? -ne 0 ]; then
        echo "⚠️ Git Pull 失敗，請查看 ../$LOG_FILE。但繼續執行..."
    else
        echo "✅ 程式碼更新完成。"
    fi
